...
const createMediaGalery = (mediaArray) => {
  ...
  mediaArray.forEach((media) => {
    ...
    const currentMedia = new MediaFactory(media)
    // On stocke le contenu HTML dans la liste
    mediaElements.push(currentMedia.createHTML)
  })

  // On l'injecte dans le HTML
  galerySection.innerHTML += mediaElements.join('')
...
const installEventLikeButton = () => {
  ...
  likesContainers.forEach((likesContainer) =>
    likesContainer.addEventListener('click', () => {
      ...
      const isButtonActive = likeButton.dataset.active
...
const handleSelect = (filteredMedias) => {
  ...
  mediaSelect.addEventListener('change', function () {
    const selectedChoice = mediaSelect.value
...
const loadMediaInLightBox = (media) => {
  ...
  const titleElement = lightBoxSection.querySelector('h2')
...
const main = async () => {
  ...
  const filteredMedias = media.filter(
